        ...
        if 'GGA' in data:
            try:
                ...
                with open(file_gps, 'a') as gps_csv:
                    gps_csv.write(str(hour) + ',' + str(global_position_sys.latitude) + ',' + str(
                        global_position_sys.longitude) + '\n')
            except pynmea2.nmea.ChecksumError as e:
                print('Parse error: {}'.format(e))
            except pynmea2.nmea.ParseError as e:
                print('Unable to parse data{}'.format(e))
...
def convert_video(config):
    ...
    for video in list_videos:
        ...
        thisCommande = BaseCommande.replace("++input_video++", input_video).replace("++input_sound++",
                                                                                    input_sound.replace(".h264",
                                                                                                        ".wav")).replace(
            "++out_put++", out_put.replace(".h264", ".mp4"))
        ...
        while page_counter == b'page2':  # page 2 /  record
            ...
            while start == b'':
                ...
                if start == b'start':  # start process of: camera, gps and distance sensor.
                    ...
                    file_video, file_sound, file_distance, file_gps = file_manager.start(id_cicliste,
                                                                                         timestamp)  # path of every file
                    ...
                    camera_process = Process(target=get_camera,
                                             args=(file_video, camera_resolution_width, camera_resolution_height,))
